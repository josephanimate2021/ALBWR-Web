                ...
                case "/randoCli": { // CLI for the ALBW Randomizer
                    ...
                    function launchExe() {
                        if (writeALBWFile(Object.assign(req, parsedUrl), {}, randoPath, false, [], ws)) {
                            const file = `albw-${parsedUrl.query.type}${parsedUrl.query.execV ? `-${parsedUrl.query.execV}` : ''}`;
                    ...
                    try {
                        ...
                        if (parsedUrl.query.command) {
                            command1 = parsedUrl.query.command.split(" ")[0];
                            command2.push(parsedUrl.query.command.substring(command1.length + 1));
                        }
                        shellInit(spawn(command1, command2, {
                            name: 'xterm-color',
                            env: process.env,
                            shell: true
                        }))
                                ...
                                case "deletePreset": {
                                    ...
                                    switch (ext) {
                                        case "json": { 
                                            ...
                                            /**
                                             * Spoiler logs in yamls will only work with versions older than z17r v0.0.9 and newer. 
...
.get('/settings/:sourceVersion/:v', (req, res) => {
    // variables
    const presets = [];
    const randoPath = path.join(__dirname, `./sourcecodes/${req.params.sourceVersion}${
...
.get('/execVersions/:sourceVersion/:v', (req, res) => {
    const versionsPath = path.join(__dirname, `./sourcecodes/${req.params.sourceVersion}${
        req.params.sourceVersion == "dev" ? `/${req.headers['x-forwarded-for'] || 'localhost'}` : ''
